...
export async function runLongHarness(opts: LongEvalOptions): Promise<LongSessionReport> {
  const turns = getTurns(opts.numRounds);
  const mode = opts.mode ?? "both";

  console.log(`\n\x1b[1mLong-Session Eval Harness\x1b[0m — ${turns.length} rounds — ${mode === "both" ? "baseline + optimized" : mode + " only"}`);
  ...
  if (mode === "baseline" || mode === "both") {
    ...
    baseline = await runSession(turns, "baseline", opts);
  }

  if (mode === "optimized" || mode === "both") {
    console.log(`\x1b[2mRunning OPTIMIZED session...\x1b[0m`);
    optimized = await runSession(turns, "optimized", opts);
  }
